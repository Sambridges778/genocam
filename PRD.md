...
3.2 后台管理平台（Admin Console）范围（MVP+）
- 目标：提供监看、风控与运营支持，保障平台稳定与合规。
- 角色：Admin（平台管理员）、Ops（运营）、Support（客服）。
- 页面：
  - Dashboard：实时概览（订单量、核销率、活跃商家、失败率）。
  - Merchants：商家资质审核、门店信息管理、状态开关。
  - Bags：礼包库与上下架、时间段与库存批量调整。
  - Orders Monitor：订单列表、状态过滤、异常重试占位。
  - Redeem Monitor：核销明细、异常核销预警、撤销占位。
  - Disputes & Refunds：申诉单、退款审批与处理规范占位。
  - Alerts & Incidents：告警规则、通知渠道、处置记录。
  - Analytics：周/月报表、基础漏斗与留存趋势。
  - Settings & RBAC：角色权限、操作范围配置、服务开关。
  - Audit Logs：审计日志（时间、操作者、对象、动作）。
- 能力：
  - 统一登录与权限（RBAC），操作审计；
  - 只读监看与只写操作区分；
  - 导出 CSV/Excel 占位；
...
- M4 评价、收藏与基础分析：评价闭环与收藏页；商家数据概览。
 - M5 后台管理平台：Dashboard、商家审核、订单/核销监看、审计日志与
   RBAC 基础上线；后续迭代补充告警与申诉/退款流程。
